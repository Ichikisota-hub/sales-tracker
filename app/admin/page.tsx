...
export default function AdminPage() {
  ...
  async function syncToSheets() {
    ...
    try {
      ...
      if (res.ok) {
        const s = d.stats
        setSyncResult({ ok: true, message: `同期完了 ✓ ${s.synced_reps}名のタブに書き込み完了${s.skipped_reps?.length > 0 ? `（タブなしのためスキップ: ${s.skipped_reps.join('、')}）` : ''}` })
      } else {
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="p-4 max-w-2xl mx-auto space-y-4 mt-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          ...
          <p className="text-slate-500 text-xs mb-4">代理店のデータをExcelファイル（.xlsx）でダウンロードします。<br />担当者・チーム・月間計画・日別実績・シフト・契約宅・日報の7シートが含まれます。</p>
          <a
            href={organization ? `/api/export?organizationId=${organization.id}` : '/api/export'}
            download
