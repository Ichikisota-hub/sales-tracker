...
export default function SheetView({ repId, repName, yearMonth }: Props) {
  ...
  // pending saves: dateStr -> field/value pairs queued for DB write
  const pendingRef = useRef<Record<string, Partial<DailyRecord>>>({})
  const saveTimer = useRef<NodeJS.Timeout | null>(null)
  const currentKey = useRef(`${repId}__${yearMonth}`)

  const days = getDaysArray(yearMonth)

  // When rep or month changes: flush immediately, then reload
  useEffect(() => {
    const newKey = `${repId}__${yearMonth}`
    if (currentKey.current !== newKey) {
      // flush any pending saves for the OLD rep/month before switching
    ...
    // Reset UI state immediately so old data doesn't flash
    setRecords({})
    setPlan(null)
    setLastSaved('')
    loadData()
  ...
  async function flushPending() {
    ...
    pendingRef.current = {}
    for (const [dateStr, fields] of Object.entries(pending)) {
      if (Object.keys(fields).length === 0) continue
      await supabase.from('daily_records').upsert(
        { ...fields, sales_rep_id: currentKey.current.split('__')[0], record_date: dateStr, updated_at: new Date().toISOString() },
  ...
  async function updateRecord(dateStr: string, field: keyof DailyRecord, value: string | number) {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      pendingRef.current = {}
      for (const [ds, fields] of Object.entries(toSave)) {
        const { data } = await supabase.from('daily_records').upsert(
          { ...fields, updated_at: new Date().toISOString() },
          { onConflict: 'sales_rep_id,record_date' }
        ).select().single()
        if (data) setRecords(prev => ({ ...prev, [ds]: data }))
      }
      setSaving(false)
            ...
            {days.map((d, idx) => {
              ...
              return (
                <tr key={d.dateStr} className={rowClass(d.dow)}>
                  ...
                  <td className="bg-yellow-50">{rec.acquisitions > 0 ? rec.acquisitions : weekend ? '•' : ''}</td>
                  <td className={progress > 0 ? 'progress-positive' : progress < 0 ? 'progress-negative' : 'progress-zero'}>
                    {isWorking ? progress : ''}
                  </td>
                  <td className="bg-blue-50">
                    <select value={rec.work_status || ''} onChange={e => updateRecord(d.dateStr, 'work_status', e.target.value)}
                  ...
                  <td style={{background:'#4472c4', color:'white'}}>
                    <select value={rec.attendance_status || ''} onChange={e => updateRecord(d.dateStr, 'attendance_status', e.target.value)}
                  ...
                  <td style={{background:'#4472c4', color:'white'}}>
                    <select value={rec.working_hours || ''} onChange={e => updateRecord(d.dateStr, 'working_hours', parseFloat(e.target.value) || 0)}
